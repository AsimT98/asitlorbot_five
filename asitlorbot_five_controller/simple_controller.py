...
class SimpleController(Node):
    ...
    def __init__(self):
        ...
        self.vel_sub_ = self.create_subscription(TwistStamped, "/asitlorbot_five_controller/cmd_vel", self.velCallback, 10)
        self.joint_sub_ = self.create_subscription(JointState,"/joint_states", self.jointCallback, 10)        
        self.odom_pub_ = self.create_publisher(Odometry, "/asitlorbot_five_controller/odom", 10)

        self.speed_conversion_ = np.array([[self.wheel_radius_/2, self.wheel_radius_/2],
                                           [self.wheel_radius_/(2*self.wheel_separation_), -self.wheel_radius_/(2*self.wheel_separation_)]])
